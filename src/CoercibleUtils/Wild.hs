...
-- | Upgrade a "packer" to a function.
--
-- >>> data Proxy2 a b = Proxy2
-- >>> upgrade (Proxy2 :: Proxy2 Bool All) True
-- All {getAll = True}
upgrade :: Coercible a b
        => (a `to` b)
        -> a
        -> b
upgrade _ = coerce
...
over2 :: (Coercible a b, Coercible a' b')
      ...
      -> b
      -> b'
-- We could use a -> a' -> a'' as we do in the Newtype module, but
-- inference would be even more horrible than it is now.
over2 _ f = coerce f
{-# INLINE over2 #-}
